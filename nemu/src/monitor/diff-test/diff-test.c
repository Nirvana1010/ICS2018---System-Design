...
void difftest_step(uint32_t eip) {
  ...
  if(r.eax != cpu.eax)
  {
    Log("different in register EAX: when nemu.eip = 0x%x", cpu.eip);
    printf("QEMU 0x%08x\n", r.eax);
    printf("NEMU 0x%08x\n", cpu.eax);
    diff = true;
  }
  if(r.ebx != cpu.ebx)
  {
    Log("different in register EBX: when nemu.eip = 0x%x", cpu.eip);
    printf("QEMU 0x%08x\n", r.ebx);
    printf("NEMU 0x%08x\n", cpu.ebx);
    diff = true;
  }
  if(r.ecx != cpu.ecx)
  {
    Log("different in register ECX: when nemu.eip = 0x%x", cpu.eip);
    printf("QEMU 0x%08x\n", r.ecx);
    printf("NEMU 0x%08x\n", cpu.ecx);
    diff = true;
  }
  if(r.edx != cpu.edx)
  {
    Log("different in register EDX: when nemu.eip = 0x%x", cpu.eip);
    printf("QEMU 0x%08x\n", r.edx);
    printf("NEMU 0x%08x\n", cpu.edx);
    diff = true;
  }
  if(r.esp != cpu.esp)
  {
    Log("different in register ESP: when nemu.eip = 0x%x", cpu.eip);
    printf("QEMU 0x%08x\n", r.esp);
    printf("NEMU 0x%08x\n", cpu.esp);
    diff = true;
  }
  if(r.ebp != cpu.ebp)
  {
    Log("different in register EBP: when nemu.eip = 0x%x", cpu.eip);
    printf("QEMU 0x%08x\n", r.ebp);
    printf("NEMU 0x%08x\n", cpu.ebp);
    diff = true;
  }
  if(r.esi != cpu.esi)
  {
    Log("different in register ESI: when nemu.eip = 0x%x", cpu.eip);
    printf("QEMU 0x%08x\n", r.esi);
    printf("NEMU 0x%08x\n", cpu.esi);
    diff = true;
  }
  if(r.edi != cpu.edi)
  {
    Log("different in register EDI: when nemu.eip = 0x%x", cpu.eip);
    printf("QEMU 0x%08x\n", r.edi);
    printf("NEMU 0x%08x\n", cpu.edi);
    diff = true;
  }
  if(r.eip != cpu.eip)
  {
    Log("different in register EIP: when nemu.eip = 0x%x", cpu.eip);
    printf("QEMU 0x%08x\n", r.eip);
    printf("NEMU 0x%08x\n", cpu.eip);
    diff = true;
  }
